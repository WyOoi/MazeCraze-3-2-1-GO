...
void scanFloorColor() {
  ...
  Serial.println("---Floor Scan Done---");
}
...
void scanTreasureColor() {
  ...
  Serial.println("---Treasure Scan Done---");
}
...
long readDistance(int i) {
  ...
  digitalWrite(trigPins[i], LOW);
  long dur = pulseIn(echoPins[i], HIGH, 30000);
  return dur * 0.034/2;}
void scanUltrasonicDFS(int idx, bool vis[]) {
  if (idx < 0 || idx >= 3 || vis[idx]) {
    return;
  }
  vis[idx] = true;
  Serial.printf("US#%d: %ldcm\n", idx, readDistance(idx));
  for (int i = 0; i < 2; i++) {
    scanUltrasonicDFS(adjacency[idx][i], vis);
  }
}

void scanWallDFS() {
  bool vis[3] = {false, false, false};
  scanUltrasonicDFS(0, vis);
  Serial.println("---US DFS Done---");
}
...
void forward(int spdL, int spdR) {
  ...
  ledcWrite(motorLeft, spdL);
  digitalWrite(IN3, HIGH);
  digitalWrite(IN4, LOW);
  ledcWrite(motorRight, spdR);
}

void reverse(int spd) {
  digitalWrite(IN1, HIGH);
  digitalWrite(IN2, LOW);
  ledcWrite(motorLeft, spd);
  digitalWrite(IN3, LOW);
  digitalWrite(IN4, HIGH);
  ledcWrite(motorRight, spd);
}
...
void setup() {
  ...
  for (uint8_t i = 0; i < 5; i++) {
    tcaSelect(i);
    if (!apds[i].begin()) {
      Serial.printf("APDS#%u fail\n", i);
    } else {
      apds[i].enableColor(true);
      Serial.printf("APDS#%u ready\n", i);
    }
    delay(100);
  }
  ...
  attachInterrupt(digitalPinToInterrupt(encA), onEncA, RISING);
  pinMode(encB, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(encB), onEncB, RISING);
}
